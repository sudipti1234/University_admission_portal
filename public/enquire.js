    ...
    .then(data => {
      if (data.availableSeats > 0) {
        ...
        fetch('/api/enquire', {
          ...
          .then(r => r.json())
          .then(d => { alert(d.message); document.getElementById('enquireForm').reset(); })
          .catch(() => alert('An error occurred while submitting the enquiry.'));
      } else {
        alert('No seats available for the selected programme and stream.');
      }
    })
    .catch(() => alert('An error occurred while checking seat availability.'));
});
